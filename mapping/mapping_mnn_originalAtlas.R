...
genemeta <- setnames(genemeta, c("new_hvg"), c(paste0("OriginalAtlas_", opts$query_batches, "_hvg")))

mapping$mapping <- get_meta.original(correct_atlas = mapping$correct_atlas,
...
mapping$mapping <- data.frame(
  ...
  celltype.mapped.original  = unlist(ct),
  stage.mapped.original     = unlist(st),
  closest.cell.original     = unlist(cm),
...
message("Reassigning Mesodermal Cells...") 
meta[,celltype.mapped.original.base := celltype.mapped.original]
atlas.mesocells <- meta_atlas[(celltype_old %in% c("Paraxial mesoderm", "Somitic mesoderm", "Intermediate mesoderm")) & (stage == "E8.5")]$cell

gastr.mesocells <- intersect(meta[(celltype.mapped.original %in% c("Paraxial mesoderm", "Somitic mesoderm", "Intermediate mesoderm"))]$cell,
...
mesomap.out$mapping <- data.frame(
  cell                          = names(mesomap), 
  celltype.mapped.original.meso = unlist(ct),
...
meta[!is.na(celltype.mapped.original.meso), celltype.mapped.original := celltype.mapped.original.meso]
